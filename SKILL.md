...
- **2026-09-05 — `SOVEREIGN` Steam emulator (offline, no Steam client needed).** Recognisable by `Game_Data/Plugins/x86_64/{SOVEREIGN64.dll, SOVEREIGN.ini, steam_api64.dll, steam_api64.svrn}` (**`.svrn` = the renamed original Valve DLL**; the `.dll` is the emulator). `SOVEREIGN.ini`:
  ...
  <3 hex lines>
  ```
  It just works under Wine — no `steam_appid.txt`, no `steam_settings/`, no GoldBerg surgery. **Language is switchable by editing `Language=`** (Steam codes: `english`, `schinese`, `japanese`, `german`, `french`, `russian`, `portuguese`); have the launcher `sed -i` it idempotently. Verify two ways: Player.log prints `[Loc] Steam language 'schinese' -> 'zh'`, and the game's PlayerPrefs registry key stores `Language_h3872303031=hex:7a,68,00` ("zh"). Change BOTH the emu `Language` and the Wine `LANG`/`LC_ALL`, or Unity's `Application.systemLanguage` and the Steam language will disagree.
- **2026-09-05 — Clone the whole `prefix/` from a sibling port, not just `wine/` + `dxvk/`.** `cp -Rc <donor.app>/Contents/Resources/{wine,dxvk,prefix} <new.app>/Contents/Resources/` (APFS clone → instant). The cloned prefix already carries the DXVK `d3d11=n,b;dxgi=n,b` DLL overrides registered in `user.reg`, plus fonts/CJK setup — no re-registration needed. Leftover `AppData/LocalLow/<other games>/` from the donor is harmless; delete stale `*.lock` files before first launch.
- **2026-09-05 — Truncate Unity's `Player.log` on every launch.** URP logs `In the render graph API, the output Render Texture must have a depth buffer…` **once per frame** (a game/URP bug, not a Wine problem) → ~4 MB/min, ~240 MB/hour. Non-fatal but sloppy. Cheapest fix in the launcher, before `exec`:
  ```zsh
  for PL in "$PREFIX"/drive_c/users/*/AppData/LocalLow/Punch*/My\ Fire*/Player.log; do
    [ -f "$PL" ] && : > "$PL"
  done
  ```
  Keeps every session's log fresh without losing diagnostics (`-nolog` would kill them entirely).
- **2026-09-05 — `pkill -f` needs the sandbox disabled, and `ps` may be denied anyway.** To restart a running game during testing, `pkill -f "<Game>.exe"` only works with `dangerouslyDisableSandbox: true`. Scope the pattern precisely (`My Fire Is Bigger Than Yours.exe` does NOT match `My Fire Is Bigger Than Yours - DEMO.exe`) so you don't kill a sibling port the user is playing.
- **2026-09-05 — ⚠️ `wine explorer /desktop=NAME,WxH <exe>` can HANG before spawning the child .exe on some Wine builds.** Observed on My Fire's bundled Wine 11.16 staging: only the `explorer` process comes up, no game window, DXVK cache is never written, and the game never renders. Symptom check: `pgrep -fl wine` shows `explorer /desktop=...` but NO second `wine <game>.exe` process. Fix/workaround: launch the exe directly (no virtual desktop), or shrink the backbuffer with Unity flags `EXTRA_FLAGS="-windowed -screen-width 1280 -screen-height 800"`. Keep the virtual-desktop path as an opt-in, default OFF.
- **2026-09-05 — On macOS, Wine typically renders at the display's LOGICAL (1x) resolution, not 2x Retina.** A fullscreen game window reported as 1512x982 by `winlist` is the real backbuffer pixel count on a 3024x1964@2x display — NOT 4x. So "Retina 2x fillrate" is rarely the cause of lag on Apple Silicon Wine ports; suspect a game-side Unity object/asset leak instead (Player.log `Loaded Objects now: <huge>` that only drops at quit, plus per-frame DOTween/URP spam). The launcher cannot patch a game-side leak — only mitigate (silence the log I/O, suggest periodic restart).
- **2026-09-05 — ⭐ `footprint` is the memory microscope for Wine ports (works when `ps`/`top`/`powermetrics` are permission-denied).** `pgrep -f "<Game>.exe"` still works for PIDs; then `footprint --noCategories -f formatted <PID>` prints `phys_footprint` (sample it twice ~3-4 min apart at a FIXED workload, e.g. idle at the main menu, to get the leak rate in MB/min), and `footprint -f formatted <PID>` (without `--noCategories`) prints a per-category breakdown that ATTRIBUTES the growth:
  | Category growing | Meaning | Fixable at port layer? |
  |---|---|---|
  | `IOAccelerator (graphics)` / `Owned physical footprint (unmapped) (graphics)` | Metal/Vulkan GPU-side allocations (DXVK↔MoltenVK churn) | YES (dxvk.conf) |
  | `WINE_RESERVE` + `untagged (VM_ALLOCATE)` | Wine native heap = Unity native/managed allocations | no (game side) |
  | `Rosetta *` | x86→ARM translation overhead | no (fixed cost) |
  Ignore one-off jumps during scene loads — they are bursts, not leaks. Needs `dangerouslyDisableSandbox: true`.
- **2026-09-05 — ⭐⭐ Unity URP games leak ~50+ MB/min under DXVK+MoltenVK even IDLE AT THE MENU — the fix is a `dxvk.conf` next to the game exe.** Mechanism: URP's render graph allocates/updates dynamic resources every frame → Vulkan descriptor pools exhaust (`VK_ERROR_OUT_OF_POOL_MEMORY: VkDescriptorPool exhausted pool of 6144 VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC descriptors, Allocating descriptor dynamically` in DXVK info logs) → **MoltenVK never recycles the dynamically-allocated descriptors** → unbounded growth (measured: phys_footprint 2489 MB @ t+14 min, 56 MB/min at menu idle → swap → "gets laggier the longer I play"). This is a known MoltenVK+DXVK pathology, NOT the game's fault. Fix — write `dxvk.conf` into the game directory (DXVK auto-loads it from the CWD; the launcher `cd`s to the game dir) and restart:
  ```
  d3d11.cachedDynamicResources = "a"   # cache dynamic buffers instead of reallocating per frame
  dxgi.maxFrameLatency = 1             # bounds live descriptor sets
  dxvk.numCompilerThreads = 4          # bound async-compile workers (DXVK-macOS async builds)
  ```
  Verify the option exists first: `strings system32/d3d11.dll | grep -i cachedDynamic` → look for `Unknown flag for d3d11.cachedDynamicResources option:` (the parser's error string proves support). **Measured result on My Fire (Unity 6 URP, DXVK-macOS async 1.10.3, M1 Pro): menu-idle leak rate 56 MB/min → 1.4 MB/min (~40x); graphics categories completely flat.** A/B methodology: same workload (menu idle), two samples 3-4 min apart, compare rates; attribute with category deltas. Do NOT judge during active gameplay (scene loads are bursty).
- **2026-09-06 — ⭐⭐ Unity intro/logo videos HANG FOREVER under Wine → black screen that looks exactly like a dead renderer.** Wine has **no working Media Foundation decoder**: `winedmo.so` needs x86_64 `libavutil.59` / `libavcodec.61` / `libavformat.61` (FFmpeg 7), which the gcenx bundle does **not** ship and Homebrew only has in newer **arm64** versions (incompatible with the x86_64/Rosetta Wine). Symptom: `Player.log` stops right after ` start play ".../StreamingAssets/video/<Logo>.mp4"`, the game window exists at full size, and pixel stats say `dark_fraction=1.0` — indistinguishable from "DXVK is broken". **Fix: move the startup `.mp4` files out of `<Game>_Data/StreamingAssets/video/`** (e.g. into a `video/_disabled/` subfolder). Unity then logs `[Critical Error] e video "..." dont found` and simply **advances to the next video / the menu** — no code change, no crack. Verified on Heroes of Might and Magic: Olden Era (Unity 6000.0.66f1 IL2CPP): disabling `Unfrozen.mp4`, `Hooded_Horse.mp4`, `Ubisoft.mp4`, `Trailer.mp4` (the whole startup chain) took it from a frozen black screen to the main menu with `dark_fraction 0.038 / 1479 color buckets / ANIMATING`. Also disable campaign movies (`Campaign_Intro.mp4`, `Campaign_Outro.mp4`) or the same hang hits mid-campaign. Tell the user the trade-off: **no in-game cinematics**.
- **2026-09-06 — Microsoft Store / MSIX / GDK Unity build: `XGameRuntime` failure is NON-FATAL.** Detect by `appxmanifest.xml` + `MicrosoftGame.config` + `Plugins/x86_64/{XGameRuntime.Thunks.dll, Microsoft.Xbox.Services.GDK.C.Thunks.dll, libHttpClient.GDK.dll}` and NO `steam_api*.dll`. `Player.log` then shows `FAILED: Initialize XGameRuntime, HResult: 0x89240101 (E_GAMERUNTIME_DLL_NOT_FOUND)` and repeated `Platform Store is not running. Please login to Store Platform and restart the game.` → `Store state changed to Failed`. **The game still boots to the main menu**, so do NOT chase Store/GDK emulation. The repack's `winmm.dll` proxy (OnlineFix, `OnlineFix.ini` carrying a `ProductId=9P2TFCCRKMRD`-style **Store** id, not a Steam appid) is what normally fakes the licence — but `winmm=b` (Wine builtin) works fine offline for single-player. Keep `USE_ONLINEFIX=1` as an opt-in only.
- **2026-09-06 — p7zip 17.05 (`7z`) cannot read modern RAR5: every entry reports `ERROR: Unsupported Method`.** `7z l` lists the archive fine (so it *looks* usable) but `7z x` silently writes 0-byte files and reports `Sub items Errors: <n>`. Use `bsdtar -xf` (libarchive) — it extracted the same 2.5 GB / 546-file RAR5 in ~90 s. Symptom to catch early: probe with `bsdtar -xf arc.rar "some/small/file"` and check `wc -c`.
...
- **2026-09-12 — `render_gate.sh` missed library-mode prefixes (false "Player.log N seconds old" warning).** Its `PREFIX_DIR` search only matched a dir literally named `prefix` directly under `~/Library/Application Support/<x>/`; the shared `GameToMac/prefixes/<id>` layout has two extra levels, so it fell through to the stale template-clone log. Fixed the glob to also scan `~/Library/Application Support/GameToMac/prefixes/*`. After the fix the gate correctly finds the per-game `Player.log` and scores the FPS meter.
- **2026-09-12 — Unity 6 Mono + GoldBerg offline emu confirmed working under Wine (no Steam login).** Zad Archery (Unity 6000.0.79f1 Mono, `steam_api64.dll` + `steam_settings/` GoldBerg repack, appid 4412000) ports cleanly with gcenx `wine-staging-11.16` + DXVK-macOS async 1.10.3, `-force-d3d11`. Window `1512x982`, `Direct3D 11.0 [level 11.0]` on Apple M1 Pro, `Loaded All Assemblies` (Mono OK), DXVK cache regenerates, pixel gate `dark_fraction=0.131 / 1388 color buckets / motion 0.0413 => PASS`. GoldBerg writes its own log inside the game dir at runtime — keep `WINEDLLOVERRIDES` `winmm=b` (builtin, do NOT force `winmm=n`) and do NOT ad-hoc codesign the bundle (seal breaks on the runtime write; use `xattr -dr com.apple.quarantine` + Open Anyway). Harmless at-boot warnings: `The referenced script ... (TutorialManager/GFX_Tail) is missing!` and `shader 'Saham/MiningRockHit' not found; juice disabled.` — both dev-leftover, ignore. Adds to the Unity 6 Mono + DXVK working set (joins My Fire, How to Fish).
- **2026-09-12 — ⭐⭐ Ship `dxvk.conf` PROACTIVELY for any Unity URP port — do NOT wait for "gets laggier the longer I play".** This is the lesson from Zad Archery above: `Player.log` showed `RenderGraph is now enabled` (URP) and I shipped the port without `dxvk.conf`; the user came back with progressive lag. From now on, `RenderGraph is now enabled` (or any `UniversalRenderPipeline` / `RenderGraph` mention in the initial render_gate's `Player.log` tail) is the **trigger to drop the three-line `dxvk.conf` into `Contents/Resources/game/` as part of the port**, not as a post-hoc fix:
  ```
  d3d11.cachedDynamicResources = "a"   # cache dynamic buffers instead of reallocating per frame
  dxgi.maxFrameLatency = 1             # bounds live descriptor sets
  dxvk.numCompilerThreads = 4          # bound async-compile workers
  ```
  Reinforces 2026-09-05 ⭐⭐ (URP RenderGraph churns Vulkan descriptors per frame → MoltenVK never recycles → ~56 MB/min). On Zad Archery, footprint A/B after the user reported lag: post-fix **6.0 MB/min** (1280→1289 MB over 90 s on M1 Pro, DXVK-macOS async 1.10.3, gcenx wine 11.16) — ~9× under the documented baseline. Category breakdown at that point: `IOAccelerator (graphics)` 205 MB + `Owned physical footprint (unmapped) (graphics)` 352 MB are the dominant absolute size but flat; the residual ~6 MB/min growth is in `WINE_RESERVE` 411 MB + `untagged (VM_ALLOCATE)` 70 MB — the Wine/Unity native heap, unfixable at the port layer. Verification gotchas: (1) **DXVK-macOS async 1.10.3 does NOT dump the parsed `dxvk.conf` into the d3d11 log even at `DXVK_LOG_LEVEL=info`** — no `d3d11.cachedDynamicResources` line appears. Do NOT use log presence to verify the fix loaded; use footprint A/B (two samples 60–90 s apart at a fixed workload like menu idle, compute ΔMB/min; `<10 MB/min` = working, `>25 MB/min` = not loaded). (2) The `strings system32/d3d11.dll | grep -i cachedDynamic` check from the original pitfall still applies to confirm the DXVK build supports the option (look for the `Unknown flag for ... option:` parser error string). (3) If the relative-load (`./dxvk.conf` from CWD = game dir) is ever uncertain on a future build, force it with `export DXVK_CONFIG_FILE="$RES/game/dxvk.conf"` in the per-user `~/Library/Application Support/<game_id>/config.sh`. Workflow change for §11: after the initial render_gate PASS, add a step "URP? → Write `Contents/Resources/game/dxvk.conf`" before marking the port done. Consider a checklist line "dxvk.conf for URP?" next to the language pin.
- **2026-09-13 — Single-file WinRAR SFX repack of a Unity game (`Pupple Pop.exe`, 128 MB) is just a wrapper — carve, don't Wine the SFX.** Detect: PE section table ends ~0x7b800 but the file is 128 MB → huge overlay; `grep --binary-files=text` finds `sfxrar.pdb` (WinRAR SFX marker). Carve the real RAR: find the `Rar!` magic (one hit, at 0x7e13c here), write everything from that offset to a `.rar`, then `bsdtar -xf` it (works for RAR5; `7z`/`unar` may be missing). Inside was a bog-standard Unity 6000.0.62f1 Mono + GoldBerg (`steam_api64.dll` + `steam_settings/` + `.xdg` renamed Valve DLL) → the §11c/Unity-Mono recipe applies unchanged. Also delete the repack's junk before packaging: `*.url` shortcuts, Chinese-named `游戏启动说明.txt`.
- **2026-09-13 — Extracting the .ico/.icns from a Unity exe's PE resources (parser gotchas):** resource dir entries are 8 bytes `[NameOrID:u32][OffsetToData:u32]`; directory header's named/id entry counts are two `u16` at offset +12 (reading 6 dwords per entry or missing the id-count gives 0 entries / garbage). RT_ICON blobs are BITMAPINFOHEADER+BGRA rows (bottom-up, h stored doubled) — decode manually with Pillow (`frombytes('RGBA',(w,w),raw[40:...],'raw','BGRA')` + FLIP_TOP_BOTTOM), then `.save(out,'ICNS')`; Pillow's own ICO plugin chokes on some AND masks, and `iconutil` is unreliable. ⚠️ Silent-truncation trap: loading only the first N KB of the exe (`read(0x80000)`) makes the big 256×256 icon blob (270 KB, lives near the end of .rsrc) slice short — `Image.frombytes` then fails with `ValueError: not enough image data`; read the WHOLE file or at least through the end of .rsrc.
- **2026-09-13 — render_gate false FAIL on first-ever launch: "no window" ≠ dead when the prefix is brand new.** On Pupple Pop's very first run the window was absent after 60 s (prefix registration + first shader compile) while the process stayed alive; a plain pkill + `open -a` relaunch brought up the full 1512x982 window. Rule: if winlist shows no window but `pgrep -f <exe>` is alive, wait/restart ONCE before declaring init death. Also fixed a real render_gate bug found this run: the prefix search took the FIRST existing dir from `~/Library/Application Support/*/prefix` (a stale sibling-port prefix) instead of matching by app name — it now scans ALL candidate prefixes and picks the Player.log whose parent dir equals the app name, `break 2` style.
- **2026-09-13 — GoldBerg achievement warnings are harmless:** `Failed to trigger achievement ACH_FIRST_POP` in Player.log right after `Steam initialized successfully` is the emu lacking achievements state; game runs fine. Same class as the missing-leaderboard noise.
- **2026-09-13 — A `.app` can LAUNCH fine with NO Info.plist — launch success ≠ bundle complete.** Pupple Pop's build block aborted midway via `set -e` (icon conversion failed first), so Info.plist + manifest were never written — yet `open -a` still launched it (LaunchServices falls back to the lone executable in `MacOS/`). Symptom: generic executable icon in Dock/Finder, `plutil -p .../Info.plist` → "no such file". Fix: always end a build with a **bundle checklist** (`Info.plist`, `Icon.icns`, `MacOS/<exe>`, `game/<exe>` all exist) before declaring done. Icon refresh recipe when the icon doesn't show: rebuild the icns with `iconutil` from a full `.iconset` (16–512 @1x/@2x via `sips -z`) — Pillow's ICNS output works but iconutil is the gold standard — then `touch <App>.app` + `lsregister -f <App>.app` + `killall Dock Finder`.
- **2026-09-13 — M1 Sonoma+ Gatekeeper refuses bare unsigned Wine bundles — "已损坏" / "无法验证开发者" dialogs on every launch.** Symptom: `codesign -dv` → `not signed at all`; `spctl -a` → `rejected / no usable signature`. Fix recipe at build time: (1) `xattr -dr com.apple.quarantine` (already part of the install step), (2) `codesign --force --deep --sign - <App>.app` (ad-hoc), (3) `lsregister -f <App>.app`. Then — crucially — re-sign on every launch: the GoldBerg / wineprefix machinery writes inside `Contents/Resources/game/...` at runtime, which invalidates the signature after a few seconds of play, and the NEXT cold start shows "damaged" again. The launcher template now does `codesign --force --sign - "$BUNDLE" 2>/dev/null || true` just before `exec wine`. Ad-hoc signing on `/Applications/*` works without sudo because `cp -Rc` keeps the bundle user-owned. Corrects the older "no codesign at all" advice — that one only applied to notarized Developer ID signing.
