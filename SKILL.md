...
- **2026-08-31 — Unity 6 (6000.0.68f1) IL2CPP Windows player + GoldBerg repack on Apple Silicon: DXVK-macOS async 1.10.3 works, no OpenGL shim needed.** Verified on `Crusaders Quest: Hero Town` (Unity 6000.0.68f1, SteamRIP/GoldBerg, M1). Same recipe as Unity 6 Mono: reuse a donor gcenx `wine-staging-11.16` bundle via APFS clone, force `-force-d3d11`, DXVK reports `Direct3D 11.0 [level 11.0]` (spoofed "NVIDIA GeForce 6800", 8191 MB VRAM), MoltenVK swapchain reaches 1512×982 borderless fullscreen. The repack uses **GoldBerg** (`steam_api64.dll` large (~7 MB), `steam_api64.dll.bak` original, `steam_settings/` present, no `OnlineFix*`). GoldBerg is fully offline, but it needs a readable `steam_appid.txt` next to the `.exe` with the real appid (`4126220` for this title). The original repack had `userdata/2126587563/0/` which means appid `0` was used — adding the correct `steam_appid.txt` makes Steam init succeed (`SteamManager:Awake()` in `Player.log`). Do NOT force `winmm=b` for GoldBerg; leave the native `steam_api64.dll` to load (WINEDLLOVERRIDES: `steam_api64=n,b` if you want to be explicit). The Steam page for this appid lists `mac: true`; a legitimate purchase is the cleaner option when available. Benign MoltenVK `VK_ERROR_OUT_OF_POOL_MEMORY` descriptor-pool warnings recur every frame under DXVK; suppress the flood with `MVK_CONFIG_LOG_LEVEL=Error` while keeping real errors visible.
