...
- **2026-09-04 — ⭐⭐ FASTEST way to get Windows Steam files: Steam console `download_depot` + reassemble (10-50x faster than SteamCMD under Wine).** The SteamCMD-in-Wine recipe (2026-09-03) works but crawls (~35 KB/s through the proxy; 156 MB took over an hour). The Steam *client* console is far quicker because it uses Steam's own CDN connection. Steps:
  1. Open the console: type `steam://open/console` in any browser address bar (launches the Steam client console).
  2. Get the depot id + manifest gid anonymously first:
     `wine steamcmd.exe +login anonymous +app_info_print <appid> +quit` → read `depots` (`oslist`, `gid`, `size`, `download`).
  3. Run: `download_depot <appid> <depotid> <gid>`
  4. Output lands in `~/Library/Application Support/Steam/depotcache/` as `<depot>_<gid>.manifest` **plus a directory of LZMA chunks** — **NOT** ready-to-run game files. You must reassemble them (step 5).
  5. Reassemble with the workspace tool: `python3 reassemble_depot.py <depot> <gid> --out <dir>`.
  **Manifest format (reverse-engineered and validated against a real Bottle Flip manifest — 939 files / 1761 chunks parsed correctly):**
  - 8-byte header (`magic = 0x71F617D0` little-endian), then **plain uncompressed protobuf** (it is NOT zlib).
  - `FileMapping`: field 1 = filename, 2 = size, 4 = sha_filename, 5 = sha_content, **6 = chunks**. Trap: public proto docs say `chunks = 7`, but depotcache manifests actually carry chunks in **field 6** as a repeated submessage — using 7 silently yields zero chunks and an empty reassembly with no error.
  - `Chunk`: 1 = sha (SHA-1, 20 bytes; the chunk filename is its lowercase hex), 2 = crc (**wire type 5 / fixed32**, not varint), 3 = offset, 4 = cb_original, 5 = cb_compressed.
  - Chunk decompression: raw **LZMA1** with `dict_size = 1<<23, pb=2, lc=3, lp=0` (SteamKit2 `LZMAWrapper` params). If `cb_compressed == cb_original` the chunk is stored uncompressed.
  - The protobuf payload has a small trailer after it — an invalid wire type means "end of payload"; stop parsing cleanly instead of raising.
  - Files with `size == 0` and zero chunks are directories — create the dir, write nothing.
  Also: `download_depot` needs the account to HOLD the licence even for a free demo (`No subscription` otherwise), so the user must claim/own the app in their library first.
