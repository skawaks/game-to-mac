...
| **Unreal Engine 5.x** (`Engine/Binaries/ThirdParty/*` + IoStore paks `*.ucas/utoc`, `*-Win64-Shipping.exe`) | Wine + DXVK-macOS 1.10.3, launch `<Game>-Win64-Shipping.exe -dx11` (D3D12 Agility SDK staged but unusable — no vkd3d) | Scratch the Ticket (UE 5.1.0, GoldBerg repack, M1 Pro) | ✅ Renders/animates first try; strip `*.pdb` + non-shipping exe (1.5GB→681MB). GoldBerg emu lives next to `steam_api64.dll` under `Engine/.../Steamworks/` and just works. UE logs to `<Game>/Saved/Logs/`, not `Player.log` |
| **WinRAR SFX single-file repack** of a Unity game (`Game.exe`, 128 MB, no sibling files) | Carve the RAR overlay at the `Rar!` magic → `bsdtar -xf` → standard Unity route | Pupple Pop (Unity 6000.0.62f1 Mono + GoldBerg, appid 4224500, M1 Pro) | ✅ D3D 11.0 [level 11.0], Steam emu init OK, pixel gate PASS (511 color buckets, ANIMATING). Shared GameToMac runtime + template-cloned prefix; icon lifted from the exe's PE `RT_ICON` resources |
